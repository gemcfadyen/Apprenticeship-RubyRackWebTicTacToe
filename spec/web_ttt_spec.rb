...
RSpec.describe WebTTT do
  ...
  it 'winning status is reported' do
    expect(players).to receive(:create_players).and_return(
      {
        PlayerSymbols::X => UnreadyPlayer.new,
        PlayerSymbols::O => AiPlayer.new(PlayerSymbols::O)
    ...
    params = {
      GameParameters::MOVE => '3',
      GameParameters::GRID => '[:X, :X, :X, :O, :O, 5, 6, 7, 8]'
    }
    ...
    expect(game_state.status).to eq "The game has been won by X"
  end

  it 'draw status is reported' do
    expect(players).to receive(:create_players).and_return(
      {
        PlayerSymbols::X => UnreadyPlayer.new,
        PlayerSymbols::O => AiPlayer.new(PlayerSymbols::O)
    ...
    params = {
      GameParameters::MOVE => '3',
      GameParameters::GRID => '[:X, :X, :O, :O, :O, :X, :X, :O, :O]'
    }

    session = {
      GameParameters::PLAYER_TYPE => '3'
    }

    web_ttt = WebTTT.new(GameParameters.new(params, session, BoardAdapter.new), players, GridFormatter.new)
    game_state = web_ttt.play

    expect(game_state.status).to eq "The game is a draw!"
  end
end
